...
import { ModifierEffect } from '@cards-ts/pocket-tcg/dist/repository/effect-types.js';
...
import { MockCardRepository } from './test-utils.js';
...
export function createGameAdapterConfig(cardRepository: CardRepository): GameAdapterConfig<ResponseMessage, Controllers> {
    return createPocketTCGAdapterConfig(cardRepository);
}
...
export function runTestGame(config: TestGameConfig): {
    ...
    const driver = gameFactory(repository).getGameDriver(players, params, [ 'TestPlayer', 'OpponentPlayer' ], preConfiguredState);
    ...
    for (let step = 0; step < maxSteps && !driver.getState().completed; step++) {
        ...
        try {
            ...
            const syncResponses = typeof getSyncResponsesFn === 'function' 
                ...
                : undefined;
            if (syncResponses) {
                for (const [ position, message ] of syncResponses) {
                    if (message) {
                        let payload, data;
            ...
            // Continue if sync response handling fails
        }
    ...
    return {
        driver: driver as unknown as GameDriver<ResponseMessage, Controllers>,
        /**
...
/**
 * Shared typed config for tests - provides properly typed GameAdapterConfig
 * so generics can be inferred without specifying type parameters.
 * Use this in test setup to avoid "requires N type argument(s)" errors.
 * 
 * @example
 * const config = getSharedTestConfig();
 * const modular = new ISMCTSModular<ResponseMessage, Controllers>(config);  // Types are inferred
 */
export function getSharedTestConfig(): GameAdapterConfig<ResponseMessage, Controllers> {
    return createPocketTCGAdapterConfig(new MockCardRepository());
}
